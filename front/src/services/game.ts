...
const updateOwnersAndRemoveIsolatedNodes = (
  newSelectionConfirmed: letterObject[],
  base: letterObject[],
  board: string[][],
  turn: string
) => {
...
const removeIsolatedNodes = (
  confirmedSelections: letterObject[],
  board: string[][],
  turn: string
) => {
  const queue = [];
    ...
    neighbors.forEach((node) => {
      if (!(`${node.row},${node.column}` in searched)) {
        ...
        const nodeIndex = nodesToCheck.findIndex(
          (obj) => obj.row === node.row && obj.column === node.column
        );
        if (nodeIndex !== -1) {
  ...
  const IsolatedWithAttachedNodes = getIsolatedNodes(
    attachedNodes,
    confirmedSelections.filter(
      (obj) => obj.owner !== turn && obj.owner !== "none"
    )
  );
...
const updateBaseWithPossibleWordTable = (
  selectedLetters: letterObject[],
  possibleWordTable: { [key: string]: string[] },
  base: letterObject[]
) => {
...
const checkIfLetterSelectionIsallowed = (
  letter: letterObject,
  board: string[][],
  ...
  if (
    possibleXpositions.includes(letter.row) &&
